...
num_negative_examples = 25000; %Higher will work strictly better, but you should start with 10000 for debugging
...
lambda = 0.00001;
X = [features_pos; features_neg]; Y = [ones(size(features_pos, 1), 1); -1 * ones(size(features_neg, 1), 1)];
[w, b] = vl_svmtrain(transpose(X), Y, lambda);
...
[bboxes, confidences, image_ids] = run_detector(test_scn_path, w, b, feature_params, size(X, 2));
