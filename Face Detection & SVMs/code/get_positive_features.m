...
% rgb2gray
	...
	feat_size = int16(31 * (feature_params.template_size / feature_params.hog_cell_size)^2);
	features_pos = zeros(num_images, feat_size);
	% Go through the images
	for i = 1:num_images
		file = sprintf('%s/%s', image_files(i).folder, image_files(i).name);
		img = single(imread(char(file))) / 255;
		if (size(img, 3) >= 2)
		...
		% populate positive features
		features_pos(i, :) = reshape(hog, [feat_size, 1]);
	end
end	